 ...
 *
 * Supported providers:
 *  - "groq"      → Groq OpenAI-compatible API (free, fast)
 *  - "gemini"    → Google Gemini REST API (free)
 *  - "openrouter" → OpenRouter OpenAI-compatible API (free models available)
 *  - "ollama"    → local Ollama server (100% free, no internet)
...
export async function isAiReady(): Promise<{ ready: boolean; provider: string; reason?: string }> {
  ...
  if (!settings.apiKey) {
    return {
      ...
      reason: `No API key set for ${preset.label}. Open Settings → AI Provider to configure it.`,
    }
  }
...
async function callOpenAiCompatible(
  ...
  if (!apiKey) {
    throw new Error(
      `API key is required for the "${provider}" provider. Open Settings → AI Provider to configure it.`,
    )
  }
  ...
  if (provider === 'openrouter') {
    ...
    headers['X-Title'] = 'Cadence Calendar'
  }

  // Use JSON mode when the system prompt asks for JSON — forces valid JSON output.
